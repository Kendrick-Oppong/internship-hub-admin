...
import { Asterisk, Sparkles, Edit, X, Loader } from "lucide-react";
...
export function ZonePropertiesPanel({
  ...
  useEffect(() => {
    const subscription = form.watch((value) => {
      onValuesChangeRef.current?.(value as ZoneFormData);
    });
    ...
    <Card className="absolute font-bai-jamjuree p-3 top-30 right-30 z-10 w-80 bg-white/95 backdrop-blur-sm border-gray-400 shadow-lg overflow-hidden flex flex-col max-h-[400px]">
      ...
      <div className="flex-1 overflow-y-auto p-1 space-y-4">
        <form
          ...
          className="space-y-4"
        >
          <fieldset disabled={isSubmitting || isDeleting} className="space-y-4">
            <FieldGroup>
              {/* Name */}
              <Controller
      ...
      <div className="p-2 border-t flex items-center gap-2">
        {onDelete && isEditing ? (
          <DeleteConfirmationDialog
            data={{
              id: initialData?.id || "active",
              name: form.getValues("name") || "this zone",
            }}
            onDelete={onDelete}
